...
export default function DirecteurPage() {
  ...
  const fetchData = async () => {
    ...
    // Super Admin : voir TOUS les ambassadeurs sans filtre de branche
    const { data: ambs } = await supabase.from("ambassadeurs").select("*").order("nom");
      ...
      <header className="bg-sbbs-blue text-white px-6 py-4 flex items-center justify-between shadow-md">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-blue-200">{directeur?.branche} · Vue complète réseau</p>
          </div>
        </div>
      ...
      <main className="max-w-5xl mx-auto px-4 py-6">
        ...
        {activeTab === "overview" && (
          <div className="space-y-4">
            <div className="card">
              <h3 className="font-bold text-sbbs-blue mb-3">📈 Performance du réseau</h3>
            ...
            <div className="card">
              ...
              {classement.length === 0 && <p className="text-gray-400 text-sm">Aucun ambassadeur dans le réseau.</p>}
            </div>
          </div>
        )}
