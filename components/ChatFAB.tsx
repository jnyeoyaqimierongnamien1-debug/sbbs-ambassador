  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end gap-2">
      ...
      <button
        ...
        className={`relative flex items-center justify-center transition-all duration-300 ${pulse ? "scale-110" : "scale-100"}`}
      ...
      >
        ...
        <div
          className="w-11 h-11 rounded-full flex items-center justify-center shadow-xl hover:scale-110 transition-transform duration-200 cursor-pointer"
          style={{
            background: unread > 0
              ? "linear-gradient(135deg, #CC0000 0%, #FF4444 100%)"
              : "linear-gradient(135deg, #1A3A6C 0%, #2563EB 100%)",
            boxShadow: unread > 0
              ? "0 6px 24px rgba(204,0,0,0.45), 0 2px 8px rgba(0,0,0,0.2)"
              : "0 6px 24px rgba(26,58,108,0.45), 0 2px 8px rgba(0,0,0,0.2)",
          }}
        >
        ...
        {unread > 0 && (
          <div className="absolute -top-1 -right-1 min-w-[18px] h-[18px] bg-sbbs-red border-2 border-white rounded-full flex items-center justify-center px-1">
            <span className="text-white text-xs font-bold leading-none">{unread > 99 ? "99+" : unread}</span>
          </div>
        )}
      </button>
    </div>
